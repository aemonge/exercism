...
pub fn root_for_team(team: Team) -> Bool {
  case team {
    Team(name:, coach: _, stats: _) if name == "Chicago Bulls" -> True
    Team(name: _, coach: Coach(name:, former_player: _), stats: _)
      if name == "Gregg Popovich"
    -> True
    Team(name: _, coach: _, stats: Stats(wins:, losses: _)) if wins >= 60 ->
      True
    Team(name: _, coach: _, stats: Stats(wins:, losses:)) if losses > wins ->
      True
    Team(name: _, coach: Coach(name: _, former_player:), stats: _) ->
      former_player
  }
}
